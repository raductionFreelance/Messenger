...
class TcpMessangerServer
{
        ...
        await using (NetworkStream stream = client.GetStream())
        {
            ...
            try
            {
                while (authenticatedUser == null)
                {
                    ...
                    if (packet.Type == MessageType.LoginRequest)
                    {
                        ...
                        if (authenticatedUser != null)
                        {
                            if (_clients.ContainsKey(authenticatedUser.Id))
                            {
                                await SendPacketAsync(stream, new MessagePacket
                                {
                                    ...
                                    Data = "Цей користувач вже в мережі",
                                });
                            ...
                            var allUsersNames = _userManager.GetAllUsersNames();
                            var usersListPacket = new MessagePacket
                            {
                                Type = MessageType.UserListResponse,
                                Data = JsonSerializer.Serialize(allUsersNames)
                            };
                            await SendPacketAsync(stream, usersListPacket);
                        }
                        else
                        {
                            await SendPacketAsync(stream, new MessagePacket
                            {
                                Type = MessageType.Error,
                                Data = "Невірний логін або пароль",
                            });
                        }
                    }
                }
                ...
                while (true)
                {
                    ...
                    {
                        string roomName = packet.Data;
                        _rooms.GetOrAdd(roomName, new List<int> { authenticatedUser.Id });
                        Console.WriteLine(
                            $"[SERVER] Кімната '{roomName}' створена користувачем {authenticatedUser.Name}");
                    }
                    else if (packet.Type == MessageType.AddUserToRoom)
                    {
                        ...
                        if (parts.Length == 2)
                        {
                            ...
                            var targetUser = _userManager.GetAllUsers().FirstOrDefault(u => u.Name == targetUserName);
                    ...
                    {
                        if (packet.ReceiverName == "Загальний чат" || string.IsNullOrEmpty(packet.ReceiverName))
                        {
                        ...
                        else if (_rooms.ContainsKey(packet.ReceiverName))
                        {
                            Console.WriteLine(
                                $"[Група '{packet.ReceiverName}'] {authenticatedUser.Name}: {packet.Data}");
                            await SendGroupMessageAsync(packet, authenticatedUser.Name);
                        }
                        else
                        {
                            Console.WriteLine(
                                $"[Приват] {authenticatedUser.Name} -> {packet.ReceiverName}: {packet.Data}");
            ...
            {
                Console.WriteLine($"[Error] Сталася помилка: {ex.Message}");
            }
            finally
            {
                if (authenticatedUser != null)
    ...
    private static async Task SendPrivateMessageAsync(MessagePacket packet, string senderName)
    {
        ...
        foreach (var pair in _clients)
        {
            ...
            if (user != null && user.Name == packet.ReceiverName)
            {
                NetworkStream clientStream = pair.Value.GetStream();
                await SendPacketAsync(clientStream, packet);
                return;
